...
def attach_event_handlers(obj):
    for f in [getattr(obj, fn) for fn in dir(obj) if hasattr(obj, fn) and callable(getattr(obj, fn))]:
...
class BaseEvent:
    def __init__(self, device:Driver=None, vector:Vector=None, element:Element=None):
        self.device = device
        self.vector = vector
...
class Write(BaseEvent):
    def __init__(self, element:Element, new_value):
        super().__init__(element=element, vector=element.vector, device=element.vector.device)
